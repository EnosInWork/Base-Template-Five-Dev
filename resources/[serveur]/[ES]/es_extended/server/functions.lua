...
ESX.RegisterCommand = function(name, group, cb, allowConsole, suggestion)
	...
	if ESX.RegisteredCommands[name] then
		print(('[es_extended] [^3Attention^7] Une commande "%s" est deja enregistrer, commande prioritaire'):format(name))
	...
	RegisterCommand(name, function(playerId, args, rawCommand)
		...
		if not command.allowConsole and playerId == 0 then
			print(('[es_extended] [^3Attention^7] %s'):format(_U('commanderror_console')))
		else
			...
			if error then
				if playerId == 0 then
					print(('[es_extended] [^3Attention^7] %s^7'):format(error))
				else
				...
				cb(xPlayer or false, args, function(msg)
					if playerId == 0 then
						print(('[es_extended] [^3Attention^7] %s^7'):format(msg))
					else
...
ESX.TriggerServerCallback = function(name, requestId, source, cb, ...)
	if ESX.ServerCallbacks[name] then
		...
		print(('[es_extended] [^3Attention^7] Le Server Callback "%s" n\'existe pas. Assurez-vous que le fichier cote serveur se charge reellement, une erreur dans ce fichier pourrait l\'empêcher de se charger.'):format(name))
	end
end
...
ESX.SavePlayer = function(xPlayer, cb)
	...
	Async.parallel(asyncTasks, function(results)
		print(('[es_extended] [^2INFO^7] Sauvegarde de [^2%s^7]'):format(xPlayer.getName()))
...
ESX.SavePlayers = function(cb)
	...
	Async.parallelLimit(asyncTasks, 8, function(results)
		print(('[es_extended] [^2INFO^7] Sauvegarde de [^2%s^7] joueur(s)'):format(#xPlayers))
		if cb then
			cb()
		end
	end)
